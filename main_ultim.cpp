...
void eliberareSpatii(int nr_linii, int nr_coloane, casuta harta[MAX_LINII][MAX_COLOANE], nod *coada, nod *ultim, int &clickuri)
{
    ...
    if(coada!=NULL)
    {
        ...
        if(coada->i>0 && harta[coada->i-1][coada->j].buton)
        {
            if(harta[coada->i-1][coada->j].val == 0)
            {
                ultim->urm = new nod;
                ultim = ultim->urm;
                ultim->i = coada->i-1;
            ...
            harta[coada->i-1][coada->j].buton = false;
            clickuri++;
        }
        if(coada->i>0 && coada->j<nr_coloane-1 && harta[coada->i-1][coada->j+1].buton)
        {
            if(harta[coada->i-1][coada->j+1].val == 0)
            {
                ultim->urm = new nod;
        ...
        if(coada->j<nr_coloane-1 && harta[coada->i][coada->j+1].buton)
        {
            if(harta[coada->i][coada->j+1].val == 0)
            {
                ultim->urm = new nod;
                ultim = ultim->urm;
                ultim->i = coada->i;
            ...
            harta[coada->i][coada->j+1].buton = false;
            clickuri++;
        }
        if(coada->i<nr_linii-1 && coada->j<nr_coloane-1 && harta[coada->i+1][coada->j+1].buton)
        {
            if(harta[coada->i+1][coada->j+1].val == 0)
            {
                ultim->urm = new nod;
        ...
        if(coada->i<nr_linii-1 && harta[coada->i+1][coada->j].buton)
        {
            if(harta[coada->i+1][coada->j].val == 0)
            {
                ultim->urm = new nod;
        ...
        if(coada->i<nr_linii-1 && coada->j>0 && harta[coada->i+1][coada->j-1].buton)
        {
            if(harta[coada->i+1][coada->j-1].val == 0)
            {
                ultim->urm = new nod;
        ...
        if(coada->j>0 && harta[coada->i][coada->j-1].buton)
        {
            if(harta[coada->i][coada->j-1].val == 0)
            {
                ultim->urm = new nod;
